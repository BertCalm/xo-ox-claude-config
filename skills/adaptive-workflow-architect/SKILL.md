...
description: "Designs AI agent workflows using Adaptive Narrative Control Theory (ANCT) — an entropy-aware architecture engine that maps workflow phases to cognitive control modes (delegate, narrate, generate), builds pipelines that know when to expand and when to compress, and produces skills that match their control strategy to the uncertainty level of each phase. Trigger words: ANCT, entropy mapping, control modes, workflow architecture, 'design a workflow', 'delegate narrate generate'. Ringleader should invoke this when planning any pipeline with 3+ phases. Pipeline Connector should invoke this when I/O wiring alone is insufficient and structural mode decisions are needed first."
version: 1.1.0
author: jac007x
origin: created
...
- **Architect a skill** for the XO_OX skill ecosystem with proper entropy management
...
## The 5-Phase Architecture Process
...
When the workflow is being designed as a skill for the XO_OX skill ecosystem:
...
## XO_OX Example Applications

| Domain | Workflow Goal | Entropy Signature | Key ANCT Insight |
|--------|--------------|-------------------|------------------|
| **New engine design** | From concept to compiled code | E5→E4→E3→E2→E1 | Concept is GENERATE (E5), DSP design is GENERATE→NARRATE (E4→E3), scaffolding is DELEGATE (E1) |
| **Guru Bin retreat** | Refine presets to transcendence | E2→E4→E3→E1 | Parameter exploration is GENERATE, selecting refinements is NARRATE, writing .xometa is DELEGATE |
| **Synth Seance** | Ghost council review | E3→E4→E3→E1 | Summoning perspectives is GENERATE, synthesizing verdict is NARRATE |
| **Preset expansion** | Fill library gaps | E2→E3→E2→E1 | Gap analysis is NARRATE, sound design is GENERATE→NARRATE, JSON output is DELEGATE |
| **Site overhaul** | Full XO-OX.org redesign | E3→E5→E4→E3→E1 | Intel gathering is NARRATE, vision is GENERATE, design is GENERATE→NARRATE, implementation is DELEGATE |
...
| Platform | How to Use |
|----------|------------|
| **Claude Code CLI** | Invoke with `/adaptive-workflow-architect`. Skills live in `~/.claude/skills/`. |
| **Ringleader (subagent)** | Ringleader dispatches AWA when planning any pipeline with 3+ phases. AWA returns the entropy map; Ringleader uses it to assign agents and sequence work. |
| **Pipeline Connector** | When `/pipeline-connector` finds that I/O wiring alone is insufficient — structural mode decisions are needed — it defers to AWA first, then wires outputs. |
| **Sisters** | `/sisters` invokes AWA during process optimization cycles when a workflow needs entropy analysis before restructuring. |
...
- **Model Recommendation:** Use Opus for E4-E5 phases (novel creative decisions,
  full architecture design sessions) — maximum reasoning depth for exploratory
  and creative work. Use Sonnet for E1-E3 phases (pattern-following, audits,
  applying pre-mapped entropy signatures to known workflow types, mechanical
  output). This aligns with Session Economist guidance: match model cost to
  actual entropy level, not habit.
...
|------------|-------------|
| **/skill-creator** | Finds the need — identifies what skill or workflow is missing. AWA designs how to build it. |
| **/flywheel** | Monitors after deployment — surfaces entropy mismatches from real-world runs. AWA failure mode analysis anticipates problems before use; flywheel confirms or refutes them post-deployment. |
| **/ringleader** | Strategic orchestrator. Invokes AWA to map entropy before dispatching agents. AWA output informs Ringleader's agent selection and sequencing. |
| **/pipeline-connector** | Tactical I/O wiring. Operates downstream of AWA — once modes are assigned, pipeline-connector handles the mechanical input/output connections between phases. |
| **/sisters** | Process optimization. Invokes AWA when a workflow needs entropy re-analysis during a continuous improvement cycle. |

> **/skill-creator** finds the need → **adaptive-workflow-architect** designs the architecture → **/pipeline-connector** wires the phases → **/flywheel** monitors and refines.
